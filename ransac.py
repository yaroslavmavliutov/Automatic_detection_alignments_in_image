...
def ransac(X, y, max_iters=1000, inlier_threshold = 3 , min_inliers=3):
    best_model = None
...
# x = []
# print(model)
# for i in range(0, np.amax(np.asarray(xys[:, 1]))):
...
for i in range(0, len(xys) - 1):
    distance = abs(model[0] * xys[:, 1][i] - xys[:, 0][i] + model[1]) / math.sqrt(model[0] ** 2 + 1)
    # print('x ', X[i])
    # print('y ', y[i])
    # print('m_c ', m_c)
    # print('d ', distance)
